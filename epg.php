...
foreach ($tv_shows as $key => $value){
   ...
   if(!isset($ONE_CHANNEL[$group])) $ONE_CHANNEL[$group] = [];

   $ONE_CHANNEL[$group][] = $value;
}
//All the values here in this variable $ONE_CHANNEL
$ONE_CHANNEL = array_values($ONE_CHANNEL);

echo '<pre>';
var_dump($ONE_CHANNEL); //get the value of the array
echo '</pre>';
exit;
$ARR_ALL=$ONE_CHANNEL;
...
EOF;
  ...
  $OUT_STR .= "<table id='tbl_epg' cellspacing='0' cellpadding='0' >\n";
    for($R=0; $R<count($ARR_ALL); $R++){
      ...
      //$channel_name=$ONE_CHANNEL["channel_name"];
      ...
      $OUT_STR .= "  <td class='td_channel'><img src='/epg/icons_nz/$channel_code.png' ><br /><span class'channel_name'>$channel_name</span></td>\n";
        for($Q=0; $Q<count($shows); $Q++){
          $one_show=$shows[$Q];
          //var_dump($shows[$Q]['title']);
          //$tvtime="3:30";//$one_show["tvtime"];
          $one_show['start'] /= 1000; 
          $tvtime = date('H:i', $one_show['start']);
          $tvshow=$one_show['title'];
          //var_dump($one_show['title']);
          if ($one_show['start'] - $one_show['end']) {
